...
<main class="main-content px-3">
    <div class="d-flex justify-content-between align-items-center mt-4 mb-3">
        <h3 class="text-muted">Staff List</h3>
                ...
                <tbody>
                    @forelse ($users as $index => $user)
                        <tr>
                            ...
                            <td><span class="badge rounded-pill text-bg-primary">@if($user->role == 2) Field Officer @elseif($user->role == 3) Modarator @endif</span></td>
                            <td>{{ $user->occupation }}</td>
                            <td>{{ optional($user->presentDivision)->name }}</td>
                            <td>{{ optional($user->presentDistrict)->name }}</td>
                            <td>
                                <a href="{{ route('admin.edit', $user->id) }}" class="btn btn-sm "><i class="bi bi-pencil-square"></i> </a>
