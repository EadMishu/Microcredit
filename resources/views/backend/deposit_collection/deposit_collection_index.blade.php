    ...
    <tbody>
        @foreach ($collections as $index => $collection)
            <tr>
                ...
                <td>
                    <a href="{{ route('deposit_collections.edit_date', $collection->date) }}" class="btn btn-sm btn-primary" title="Edit">
                        <i class="bi bi-pencil-square"></i>
                    </a>

                    <form action="{{ route('deposit_collections.destroy_date', $collection->date) }}" method="POST" style="display:inline-block;">
                        @csrf
                        @method('DELETE')
